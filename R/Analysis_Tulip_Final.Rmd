...
output_gee <- output_gee %>%
  mutate(term = rownames(sum_gee$coefficients)) %>%
  relocate(term)
...
res_plot <- output_gee2 |>
  ...
  mutate(
    adjust_right_p_value_gee = case_when(
  ...
  bind_rows(data.frame(
    term = "Compound", estimate_lab = "Effect Estimate (95% CI)", conf.low_gee = "", conf.high_gee = "",
...
res_plot <- output_gee_l2 |>
  ...
  bind_rows(data.frame(
    term = "Compound", estimate_lab = "Odds Ratio (95% CI)", conf.low_gee_l = "", conf.high_gee_l = "",
